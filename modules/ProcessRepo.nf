process ProcessRepo {
    ...
    if find repo -maxdepth 1 -type f -iname '*requirements*' | grep -q .; then
       DEP_STATUS="PASS"
    elif [ -f repo/Pipfile ] || [ -f repo/Pipfile.lock ] || [ -f repo/setup.py ] || [ -f repo/pyproject.toml ] || [ -f repo/package.json ] || [ -f repo/package-lock.json ] || [ -f repo/yarn.lock ] || [ -f repo/pom.xml ] || [ -f repo/build.gradle ] || [ -f repo/settings.gradle ] || [ -f repo/DESCRIPTION ] || [ -f repo/renv.lock ] || ( [ -d repo/packrat ] && [ -f repo/packrat/packrat.lock ] ); then
       DEP_STATUS="PASS"
    fi
    ...
    # Write out a summary status file in CSV format (columns: repo_name, clone_status, dep_status, tests_status)
    mkdir -p ${out_dir}
