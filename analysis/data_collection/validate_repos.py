...
def validate_repo_url(url: str) -> tuple[bool, str]:
    ...
    if not re.match(pattern, url):
        return (
            False,
            f"Invalid format (expected: https://github.com/owner/repo.git or http://..., got: {url})",
        )
...
def validate_csv(input_file: str, output_file: str = None) -> dict:
    ...
    try:
        with open(input_file) as f:
            ...
            for row_num, row in enumerate(reader, start=2):
                ...
                if is_valid:
                    ...
                    stats["errors"].append({"row": row_num, "url": url, "error": error_msg})
        ...
        print(
            f"Invalid URLs:   {stats['invalid']} ({stats['invalid'] / stats['total'] * 100:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--output", default=None, help="Output CSV file with only valid URLs (optional)"
    )
    parser.add_argument(
        "--fix", action="store_true", help="Attempt to fix common URL format issues"
    )
