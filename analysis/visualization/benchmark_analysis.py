                ...
                if isinstance(item, dict):
                    ...
                    if name == "repo-almanack-score":
                        ...
                        if isinstance(score_data, dict):
                            ...
                            result["almanack_numerator"] = score_data.get(
                                "almanack-score-numerator", None
                            )
                            result["almanack_denominator"] = score_data.get(
...
def load_allowed_tools(repos_csv: str) -> dict:
    ...
    print(
        f"Loaded {len(allowed_tools)} unique tools from repos CSV ({len(all_variations)} total variations)"
    )
    return allowed_tools
...
def is_tool_allowed(tool_name: str, allowed_tools: dict) -> bool:
    ...
    if not allowed_tools:
        return True
    normalized = normalize_tool_name(tool_name)
    return normalized in allowed_tools or tool_name.lower() in {
        v for vs in allowed_tools.values() for v in vs
    }
...
def aggregate_results(
    ...
    for tool_name, csv_info in csv_data.items():
        ...
        if (
            "almanack_score" not in tool_data[tool_name]
            or tool_data[tool_name]["almanack_score"] is None
        ):
        ...
        if (
            "joss_overall_score" not in tool_data[tool_name]
            or tool_data[tool_name]["joss_overall_score"] is None
        ):
...
def infer_domain(tool_name: str) -> str:
    ...
    if any(
        ...
        term in tool_lower
        for term in ["phylogen", "tree", "evolution", "phylo", "raxml", "iqtree", "beast", "phylip"]
    ):
        ...
        term in tool_lower
        for term in ["epigen", "methyl", "chip", "atac", "hic", "chromatin", "histone", "bisulfite"]
    ):
        return "Epigenomics"
    else:
        return "Other"
...
def infer_language(tool_name: str, repo_path: str | None = None) -> str:
    ...
    if (
        tool_lower.startswith("py")
        or "python" in tool_lower
        or tool_lower.startswith("biopython")
        or tool_lower.startswith("scipy")
        or tool_lower.startswith("pandas")
        or tool_lower.startswith("numpy")
        or "pysam" in tool_lower
        or "pybedtools" in tool_lower
        or "scanpy" in tool_lower
        or "seaborn" in tool_lower
        or "matplotlib" in tool_lower
        or "scikit" in tool_lower
    ):
        return "Python"
    elif (
        tool_lower.startswith("r-")
        or tool_lower.endswith(".r")
        or "bioconductor" in tool_lower
        or "bioc" in tool_lower
        or "seurat" in tool_lower
        or "biostrings" in tool_lower
        or "genomicranges" in tool_lower
        or "deseq" in tool_lower
        or "limma" in tool_lower
        or "edgeR" in tool_lower
        or tool_lower.endswith("r")
        and len(tool_lower) < 10
    ):
        return "R"
        ...
        term in tool_lower
        for term in ["htslib", "samtools", "bcftools", "bwa", "bowtie", "minimap", "bwa-mem"]
    ):
        return "C/C++"
...
def perform_benchmarking_analysis(df: pd.DataFrame) -> dict:
    ...
    df["maturity"] = df.apply(
        lambda row: infer_maturity(row.get("days_of_development"), row.get("commits")), axis=1
    )
...
"""
    ...
    for stratum in ["domain", "language", "maturity"]:
        if stratum in tool_data:
            ...
            if stratum_value and stratum_value in analysis_results["stratified_statistics"].get(
                stratum, {}
            ):
...
"""
    ...
    top_tools = df.nlargest(10, "almanack_score_norm")[
        ["tool_name", "almanack_score_norm", "almanack_grade"]
    ]
    for _, row in top_tools.iterrows():
        report += (
            f"| {row['tool_name']} | {row['almanack_score_norm']:.3f} | {row['almanack_grade']} |\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--results_dir", type=str, default="benchmark_results", help="Local directory for results"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="CSV file with repo URLs to include (must have repo_url column)",
    )
    parser.add_argument(
        "--output_dir", type=str, default="benchmark_analysis", help="Output directory for reports"
    )
    ...
    parser.add_argument(
        "--skip_download", action="store_true", help="Skip S3 download if results already exist"
    )
