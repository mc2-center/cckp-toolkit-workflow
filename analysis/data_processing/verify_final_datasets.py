...
def verify_datasets():
    ...
    print(f"\nDomain coverage:")
    print(
        f"  With domain labels: {full['domain'].notna().sum():,} ({100 * full['domain'].notna().sum() / len(full):.1f}%)"
    )
...
Stars and mentions:
  ...
  With >0 mentions: {(full["repo_software_mentions_count"] > 0).sum():,}""")
    ...
    original_others = pd.read_csv(
        RESULTS_DIR / "combined_almanack_with_czi_classified.csv", low_memory=False
    )
