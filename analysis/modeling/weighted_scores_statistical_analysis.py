...
def main():
    parser = argparse.ArgumentParser(
        description="ANOVA and nf-core comparison for weighted Almanack scores"
    )
    parser.add_argument(
        "--weighted_csv",
        type=str,
    ...
    parser.add_argument(
        "--output_dir", type=str, default="data/final_results", help="Output directory"
    )
    ...
    for score_col, score_label in SCORE_CONFIGS:
        ...
        n_sig_bonf = (
            pairwise["bonferroni_significant"].sum()
            if "bonferroni_significant" in pairwise.columns and len(pairwise)
            else 0
        )
        if len(pairwise) > 0:
        ...
        plot_domain_boxplot(
            sub, score_col, score_label, out_dir / f"domain_boxplot_{safe_name}.png"
        )
    ...
    if all_pairwise:
        ...
        pairwise_combined.to_csv(
            out_dir / "weighted_pairwise_by_domain_all_scores.csv", index=False
        )
        ...
        if orig and orig.get("f_stat") is not None:
            for r in results_anova:
                ...
                if "Original" in r["score"]:
                    report_lines.append(
                        f"- **{r['score']}:** F = {r['f_stat']:.3f}, p = {r['pvalue']:.2e} (reference)."
                    )
                else:
                    bigger = "**Larger**" if r["f_stat"] > orig["f_stat"] else "Smaller"
                    report_lines.append(
                        f"- **{r['score']}:** F = {r['f_stat']:.3f}, p = {r['pvalue']:.2e} — F-statistic is {bigger} than original."
                    )
            report_lines.append("")
    report_lines.append(
        "Higher F-statistic indicates stronger domain differences. More Bonferroni-significant pairwise comparisons also indicate stronger separation by domain."
    )
